...
export type Props = { entry: Entry | null };
const props = $props<{ entry: Entry | null }>();
...
const initialEditorState = createEditorState(props.entry);
let editor = $state<EditorState>(initialEditorState);
let editorKey = $state<string>(createEditorKey(initialEditorState));
let loadedEntrySignature = $state<string>(createEntrySignature(props.entry));
	...
	function applyEntry(next: Entry | null) {
		const snapshot = createEditorState(next);
		editor = snapshot;
	editorKey = createEditorKey(snapshot);
	loadedEntrySignature = createEntrySignature(next);
}
...
$effect(() => {
	const signature = createEntrySignature(props.entry);
...
function htmlPreview(node: HTMLElement, html: string) {
	...
	async function saveEntry() {
		...
		try {
			...
			const { entry: saved } = (await response.json()) as { entry: Entry };
			applyEntry(saved);
			if (wasNew && saved.id) {
				goto(`/${saved.id}`);
			}
...
<div class="page-layout">
	...
	<!-- {#if dev}
		<section class="debug-pane">
			<h2>Debug</h2>
			<SuperDebug label="Entry prop" data={props.entry} status={false} />
